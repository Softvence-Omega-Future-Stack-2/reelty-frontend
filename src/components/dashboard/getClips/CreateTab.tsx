...
const CreateTab = () => {
  ...
  const connectWebSocket = (projectId: string, attempt: number = 0) => {
    try {
      // Use the appropriate WebSocket schema and host based on the AI API env variable
      const apiHost = new URL(import.meta.env.VITE_AI_API || "https://ai.reelty.com.au/ai/generate");
      const wsProtocol = apiHost.protocol === 'https:' ? 'wss:' : 'ws:';

      // When running against the IP-based staging backend (like 184.105.4.166:8000)
      // we need to use the full path /ai/ws/connect/ because that's where the WebSocket
      // endpoint is mounted in FastAPI, despite what the Swagger response might imply.
      const path = apiHost.hostname.includes('reelty.com.au')
        ? `/ai/ws/connect/${projectId}`
        : `/ai/ws/connect/${projectId}`;

      const wsUrl = `${wsProtocol}//${apiHost.host}${path}`;

      console.log(`Connecting to WebSocket: ${wsUrl} (Attempt: ${attempt + 1})`);
      ...
      socket.onmessage = (event) => {
        try {
          ...
          // Handle keepalive
            ...
            console.log("Keepalive received:", data);
            ...
            // Send a ping/pong response back so proxy doesn't drop due to inactivity
            if (socket.readyState === WebSocket.OPEN) {
              socket.send(JSON.stringify({ action: "pong", project_id: projectId }));
            }
          }
      ...
      socket.onclose = (event) => {
        ...
        if (event.code !== 1000) {
          ...
          if (attempt < 5) {
            console.log(`Reconnecting in 3 seconds... (Attempt ${attempt + 1}/5)`);
            setTimeout(() => {
              connectWebSocket(projectId, attempt + 1);
            }, 3000);
          } else {
